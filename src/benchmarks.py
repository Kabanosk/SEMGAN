...
def calculate_pesq(clean_signal: np.ndarray, enhanced_signal: np.ndarray, fs: int) -> float:
...
def benchmark(clean_wav_path: str, enhanced_wav_path: str) -> (float, float):
    ...
    enhanced_signal = enhanced_signal.astype(np.float32) / np.max(np.abs(enhanced_signal))
...
if __name__ == "__main__":
    pesq_result, stoi_result = benchmark(
        "data/clean.wav",
        "data/enhanced.wav"
    )
